...
> Use `extract_if` for conditional extraction (Vec: Rust 1.87+, HashMap/HashSet: Rust 1.88+)

**Rule**: `perf-extract-if`

## Why It Matters

`Vec::extract_if` (Rust 1.87), `HashMap::extract_if` and `HashSet::extract_if` (Rust 1.88) allow removing elements matching a predicate while moving them into an iterator. Unlike the manual `retain` + `clone` + `collect` pattern, `extract_if` does a single pass, avoids cloning, and reuses the existing allocation. (`BTreeMap::extract_if` remains nightly-only as of Rust 1.96.)
...
| `BTreeMap<K,V>` | `.extract_if(pred)` | nightly | not yet stable (use `.retain()` workaround) |
